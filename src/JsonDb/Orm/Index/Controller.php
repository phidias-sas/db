...
use Phidias\Db\JsonDb\Orm\Index\Entity as Index;
...
class Controller
{
    ...
    public static function filterCollection($collection, $tableId, $keyName, $keyValue)
    {
        ...
        $collection->where("id IN (SELECT `recordId` FROM `sophia_orm_table_indexes` WHERE `tableId` = :tableId AND `keyName` = :keyName AND $valueCondition)", [
            "tableId" => $tableId,
            "keyName" => $keyName,
            "keyValue" => $keyValue,
        ]);
